...
class FilmspiderSpider(scrapy.Spider):
    ...
    def parse_film(self, response):
            ...
            if table_rows.css('span[data-wikidata-property-id="P577"] ::text').get() is None:
                 ...
                 year = table_rows.css('span[data-wikidata-property-id="P577"] ::text').get()

            if table_rows.css('span[data-wikidata-property-id="P57"] ::text').getall() is None:
               director = table_rows[2].css('a[class="external text"] ::text').getall()
            else:
               director = table_rows.css('span[data-wikidata-property-id="P57"] ::text').getall()

            yield {
                 'title' : table_rows[0].css('::text').get().strip('"'),
